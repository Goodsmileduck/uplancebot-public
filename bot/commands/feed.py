...
async def add_feed(chat, msg):
    ...
    if not re.match("^[a-zA-Z0-9_]*$", name):
        ...
        return await chat.send_text('Please do not use special symbols or space.\n'
                                    'Try enter name again.')
    c = await db.chats.count_documents({})
    if c >= settings.GLOBAL_MAX_FEED_COUNT:
        return await chat.send_text('You have reached global limit.\n\
Only {} searches could be saved globaly.\n\
Please delete some using command /delete_feed'.format(settings.GLOBAL_MAX_FEED_COUNT))
...
async def delete_feed(chat, msg):
    name = chat.message.get('text', '').strip()
    d = db.chats.find({'chat_id': chat.id, 'tag': {"$exists": True} })
    list_d = []
    for i in await d.to_list(length=settings.MAX_FEED_COUNT):
        list_d.append(i['tag'])
    str_list_d = ', '.join(list_d)
    if list_d is None:
        return await chat.send_text('You don\'t have any feeds. Use /add_feed to add new feed.')
    if name == "/delete_feed":
...
async def list_feeds(chat, msg):
    feed = db.chats.find({'chat_id': chat.id, "tag": {"$exists": True} })
    message = ""
    if feed is None:
        return await chat.send_text('You don\'t have any feeds. Use /add_feed to add new feed.')
    else:
...
Also you can check list of feed with command /list_feeds \n\
and add new feeds with /add_feed'.format(name['name'], settings.MIN_UPDATE_PERIOD)
