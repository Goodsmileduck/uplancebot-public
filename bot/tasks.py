...
DIFF_TIME = Summary("delta_time_seconds",
                     "time spent between post and notification",
                     ['chat_id'])
...
async def update_upwork_status():
    if await get_upwork_status(bot):
        logging.debug('Upwork is up.')
    else:
        logging.info('Upwork is down.')
...
async def notify(chat, r, now):
    ...
    if length > 0:
        ...
        logging.info(f"Found {length} new entries for chat_id {chat['chat_id']} for feed: {chat['tag']}")
        upd = {'$set': {'last_updated': now, 'feed_updated': feed_updated}}
        await db.chats.update_one({'chat_id': chat['chat_id'], 'tag': chat['tag']}, upd)
    else:
        logging.info(f"Not found entries for chat_id {chat['chat_id']} for feed: {chat['tag']}")
    ...
    for x in entries:
        ...
        diff_time = (dt.now().timestamp() - timegm(x['updated_parsed']) )
        if diff_time > (int(chat['period']) * 5):
            logging.warning(f"Time lag is big: {diff_time} for period: {chat['period']}")
        else:
            logging.debug(f'diff time: {diff_time}')
        DIFF_TIME_SUM = DIFF_TIME.labels(chat_id=chat['chat_id'])
        DIFF_TIME_SUM.observe(diff_time)
        #await aio.time(DIFF_TIME)
